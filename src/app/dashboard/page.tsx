  ...
  return (
    <div className="space-y-12 max-w-7xl mx-auto py-8 px-4">
      ...
      <div className="bg-[#0a0a0a] rounded-3xl p-8 md:p-10 shadow-2xl border border-white/10 flex flex-col md:flex-row justify-between items-center gap-8 relative overflow-hidden">
        {/* Glow */}
        <div className="absolute top-0 right-1/4 w-64 h-64 bg-accent-yellow/10 rounded-full blur-[100px] pointer-events-none"></div>

        <div className="flex flex-col md:flex-row items-center gap-8 text-center md:text-left relative z-10">
          <div className="h-24 w-24 bg-accent-yellow text-background text-4xl font-black rounded-full flex mx-auto md:mx-0 items-center justify-center shadow-[0_0_30px_rgba(250,204,21,0.3)]">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {/* Quick Nav */}
        <Link href="/events/games" className="group bg-[#111] p-8 rounded-3xl border border-white/5 hover:border-accent-yellow/50 transition flex items-center gap-6 relative overflow-hidden">
          ...
          <div className="h-16 w-16 bg-white/5 text-white rounded-2xl flex items-center justify-center group-hover:bg-accent-yellow group-hover:text-background transition-colors shadow-lg z-10">
             <Trophy size={28} />
          </div>
          <div className="z-10">
            <h3 className="font-bold text-xl text-white group-hover:text-accent-yellow transition uppercase tracking-wide">Tournaments</h3>
            <p className="text-sm text-gray-500 font-medium">View sports & esports</p>
          </div>
        </Link>
        <Link href="/events/cultural" className="group bg-[#111] p-8 rounded-3xl border border-white/5 hover:border-accent-yellow/50 transition flex items-center gap-6 relative overflow-hidden">
          ...
          <div className="z-10">
            <h3 className="font-bold text-xl text-white group-hover:text-accent-yellow transition uppercase tracking-wide">Cultural Lineup</h3>
            <p className="text-sm text-gray-500 font-medium">View fests & drama</p>
          </div>
        </Link>
        <Link href="/my-events" className="group bg-[#111] p-8 rounded-3xl border border-white/5 hover:border-accent-yellow/50 transition flex items-center gap-6 relative overflow-hidden">
          ...
          <div className="z-10">
            <h3 className="font-bold text-xl text-white group-hover:text-accent-yellow transition uppercase tracking-wide">My Passes</h3>
      ...
      <div className="pt-8 border-t border-white/10">
         <div className="flex justify-between items-end mb-10">
            <div>
              <span className="text-accent-yellow text-sm font-bold tracking-widest uppercase block mb-2">Your Activity</span>
              <h2 className="text-3xl font-black text-white uppercase tracking-tighter">Recent Passes</h2>
            </div>
            {myRegistrations.length > 0 && (
              <Link href="/my-events" className="text-gray-400 font-bold hover:text-white transition uppercase tracking-widest text-sm border-b border-transparent hover:border-accent-yellow pb-1">View all →</Link>
            )}
         </div>
         {myRegistrations.length === 0 ? (
            <div className="bg-[#111] border border-white/5 rounded-3xl p-12 text-center flex flex-col items-center justify-center min-h-[300px]">
              ...
              <Link href="/events/cultural" className="inline-flex bg-accent-yellow text-background font-bold px-8 py-4 rounded-full shadow-[0_0_20px_rgba(250,204,21,0.2)] hover:shadow-[0_0_30px_rgba(250,204,21,0.4)] transition-all uppercase tracking-widest">Explore Lineup</Link>
            </div>
         ) : (
