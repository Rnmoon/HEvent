...
export default async function EducationalEventsPage() {
  ...
  try {
    ...
    console.error('Failed to fetch educational events:', error)
  }
  ...
  if (user) {
    try {
      ...
      console.error('Failed to fetch user registrations for educational events:', error)
    }
  }
    ...
    <div className="flex flex-col w-full -mt-8">
      ...
      <section className="relative w-full pt-40 pb-32 mb-16 overflow-hidden">
        <div className="absolute inset-0 z-0">
          <img src="/hero.png" alt="Educational Events" className="w-full h-full object-cover" />
          <div className="absolute inset-0 bg-background/80 backdrop-blur-sm"></div>
          <div className="absolute inset-0 bg-gradient-to-t from-background to-transparent"></div>
        </div>
        
        <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
          <span className="text-accent-blue text-sm font-bold tracking-widest uppercase block mb-4">Innovation & Learning</span>
          ...
          <p className="mt-6 text-xl text-gray-300 font-medium">
            Expand your horizons with technical projects, endurance challenges, and innovative showcases.
          </p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 pb-24 relative z-10">
        ...
        {events.length === 0 && (
           <div className="text-center py-32 text-gray-400 bg-white/5 rounded-3xl border border-white/10 backdrop-blur-sm">
              <h3 className="text-2xl font-bold text-white mb-2">Knowledge is loading...</h3>
              <p>Check back later for new educational and technical event announcements.</p>
           </div>
        )}
      </div>
    </div>
